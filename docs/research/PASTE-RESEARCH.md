...
- ✅ **対象**: macOS 14.0+ シングルプラットフォームMVP
- ✅ **ライセンス**: MIT（完全OSS、GitHub公開）
- ✅ **配布**: GitHub Releases (`.dmg`) を URL 共有、Developer ID公証必須
- ✅ **価格**: 完全無料、全機能解放
- ✅ **スコープ**: P0-P5 (6〜8週間) で v1.0 GA
- ✅ **同期/iOS**: v1.0には含めず（P6以降）

## 12. すぐ着手すべき次のアクション

1. **Apple Developer Program加入**（¥12,800/年）← URL配布の公証に必須
2. **Xcode project初期化** + Swift Package Manager 依存設定
3. **GRDB schema 設計** + `PasteboardObserver` プロトタイプ
4. **HotKey + Accessibility で `⌘V` 自動貼付**のスパイク
5. **GitHub リポジトリ初期化** (LICENSE: MIT, README, CONTRIBUTING)
